...
export default function AdminDashboard() {
  ...
  const [vocabSets, setVocabSets] = useState([{ id: "v1", name: "4주차 단어", createdAt: "8월 18일", assignedCount: 3 }]);
  ...
  useEffect(() => {
    async function loadMembers() {
      ...
      if (studentsError || classesError || homeworkError || attendanceError) {
        showToast("회원 데이터를 불러오지 못했어요");
        return;
      }
      setStudents((studentRows || []).map((student) => ({ ...student, classId: student.class_id })));
      setClasses(classRows || []);
      setAttendance((attendanceRows || []).reduce((result, row) => ({
        ...result,
  ...
  function copyReportLink(student) {
    const url = `${window.location.origin}/report?key=${student.report_token}`;
  ...
  async function checkAttendance(studentId) {
    const key = todayKey();
    const { error } = await supabase.rpc("admin_check_attendance", { p_student_id: studentId, p_attendance_date: key });
    if (error) {
      showToast("출석을 저장하지 못했어요");
      return;
    }
    setAttendance((prev) => {
      const list = prev[studentId] || [];
      if (list.includes(key)) return prev;
      return { ...prev, [studentId]: [...list, key] };
    });
    showToast("출석을 체크했어요");
  }
              ...
              {approvedStudents.map((s) => {
                ...
                return (
                  <div className="attend-row" key={s.id}>
                    ...
                    <button className={`attend-btn ${checkedToday ? "done" : ""}`} disabled={checkedToday} onClick={() => checkAttendance(s.id)}>
                      {checkedToday ? "오늘 체크완료" : "오늘 출석 체크"}
                    </button>
                  </div>
                );
              })}
